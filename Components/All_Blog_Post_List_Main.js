...
import { Category_Blogs_Details, Category_Blogs_Details_Back } from "../Redux/Action/Fech_Api.js";
...
const Blog_Main = (props) => {
  ...
  useEffect(() => {
    dispatch(Category_Blogs_Details(props.Detailes));
    return () => {
      dispatch(Category_Blogs_Details_Back());
    };
  ...
  if (data) {
    if (data.length !== 0) {
      ...
      return (
        <>
          <h1 className="text-warning text-center mt-2">Now No Content Available For This Topic</h1>
          <br />
          <h1 className="text-warning text-center mt-2">Now No Content Available For This Topic</h1>
          <br />
          <h1 className="text-warning text-center mt-2">Now No Content Available For This Topic</h1>
