...
if (loading) {
    return(
      <>
      <Crousel/>
      <FeaturedArticles Featured={Featured} All={All} />
      <Explore/>
      <RecentlyPublisheds Recent={Recent} />
      </>
      );
  } else {
    return(
...
export async function getServerSideProps(context){
  ...
  const Featured = await 
        axios.get("https://technicalknowledge-backends.herokuapp.com/Blogs/Read/Featured")
  const All = await 
        axios.get("https://technicalknowledge-backends.herokuapp.com/Blogs/Read/All")
  return{
    props: {
      Recent: Recent.data.Blogs,
      Featured: Featured.data.Blogs,
      All: All.data.Blogs
    }
  }
}